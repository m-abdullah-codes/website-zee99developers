...
import Paths, { PathGate, PathProvider } from "@/components/brochure/Paths";
 ...
 * plates, plus the sections that only ever existed in the light brochure (the
 * typical floor, the specification, the roof, the amenity split, the shop-by-
 * shop ledger and the builder's record), and the full tour in place of the
 * rental projection.
 *
 ...
 *
 *   3. **It asks one question and shapes itself around the answer.** The
 *      document runs in three parts: what is true for everyone (the cover and
 *      the overview), the half the reader chose (apartments *or* shops), then
 *      everything that is true for everyone again — the film, the amenities,
 *      the address, the record, the questions, and the roof at the end of it.
 *      See `components/brochure/Paths.tsx` for why, and for the three ways back
 *      out of a branch.
 *
 * Folios follow that shape. The shared spine is numbered 01 to 09; the two
 * branches are lettered R and C, because they are alternatives to each other
 * rather than steps in one sequence, and numbering them would promise a reader
 * sections they will never see.
 *
    ...
    <>
      ...
      <main>
        ...
        {/* One currency for the whole document: the plans, the shops and the
            ...
            One choice, too — the rail's switch and the fork's cards are the
            same state, so the rail is inside the provider with them. */}
        <PathProvider>
          <CurrencyProvider>
            <BrochureNav />

            {/* 01 — overview */}
            <section id="overview" className="scroll-mt-24 bg-paper py-24 md:py-32">
            ...
            {/* 02 — the fork, and the two halves of the building behind it */}
            <Paths
              no="02"
              residential={
                <>
                  {/* R1 — the plate the three plans are cut out of, ahead of
              ...
              commercial={<Shopfront no="C1" />}
            />

            {/* Everything from here down is common ground — and none of it is
                on the page until the fork above has been answered. See
                `PathGate`: the reader is not stopped from scrolling, they are
                given nothing to scroll to, because a document that lets you
                past the question answers it for you by showing you the other
                half's prices first. */}
            <PathGate>
              {/* 03 — the tour, in place of the rental projection the site runs
                  here */}
              <Film no="03" />

              {/* 04 */}
              <Amenities items={project.amenities!} media={project.amenityMedia} no="04" fold />

              {/* 05 — location */}
              <section id="location" className="scroll-mt-24 border-t border-ink/10 bg-paper py-24 md:py-32">
              ...
              {/* 09 — the roof, last. It is the one section that is not an
                  argument: the questions are answered, the record is on the
                  table, and what is left is what living here is like at seven in
                  the evening. A document that ends on a fire pit ends better than
                  one that ends on a FAQ. */}
              <Building no="09" />
